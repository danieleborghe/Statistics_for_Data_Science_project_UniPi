...
#
# Purpose:
# This script executes the Conformalizing Scalar Uncertainty Estimates experiment (Paper Section 2.3).
# It implements the method where uncertainty is modeled by predicting the magnitude of the
# absolute residuals of a primary prediction model. The script follows the standard project
# structure: multiple runs for coverage distribution and a single, detailed evaluation.
#
# Structure:
...
#          - Iteration X: Model Training (Primary + Uncertainty)
...
cat("INFO: Sourcing shared R scripts for Experiment (Section 2.3 - Scalar Uncertainty)...\n")
source("R/conformal_predictors.R")
source("R/evaluation_utils.R")
source("R/experimentation_utils.R")

cat("INFO: --- Starting Experiment: Conformalizing Scalar Uncertainty (Section 2.3) ---\n")
...
plot_coverage_histogram(all_empirical_coverages_su, ALPHA_CONF, N_RUNS, "Scalar Uncertainty Method")
dev.off()
...
cat("\nINFO: --- End of Scalar Uncertainty Experiment ---\n")